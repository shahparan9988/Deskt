...
namespace Racssoft_Dokan.Pages
{
	public partial class Products : Common
	{
		...
		Dictionary<int, string> allCategory;
		Dictionary<int, string> allBrand;
...
//			DataTable dt = (DataTable)Select(sql).Data;
			...
			int rowCount = dtVat.Rows.Count;
			if (rowCount > 0)
			{
				decimal vatValue = Convert.ToDecimal(dtVat.Rows[0]["Vat"]);
				txtVatAll.Value = vatValue;
				int vatTypeId = Convert.ToInt32(dtVat.Rows[0]["VatTypeId"]);
				if (vatTypeId == 1)
				{
					radioBtnDutyFree.Checked = true;
				}
				else if (vatTypeId == 2)
				{
					radioBtnExclusive.Checked = true;
				}
				else radioBtnInclusive.Checked = true;
			}
			lblMessage.Text = "";
			...
			txtInvoice.Text = "PINV-" + (getInventoryId() + 1);
	  ...
	  .ToDictionary<DataRow, int, string>(row => row.Field<int>(0),
		...
		void lbAutoComplete_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			if (lbAutoComplete.SelectedIndex > -1)
			{
				...
				string sql = "SELECT c.Title, b.Title FROM ((Products p LEFT JOIN Category c ON p.CategoryId = c.ID) LEFT JOIN Brand b ON b.ID = p.BrandId) WHERE p.ID = " + productId + "";
				DataTable dt = (DataTable)Select(sql).Data;
				int count = 0;
				count = dt.Rows.Count;
				if (count > 0)
				{
					txtCategory.Text = Convert.ToString(dt.Rows[0]["c.Title"]);
					txtBrand.Text = Convert.ToString(dt.Rows[0]["b.Title"]);
				}
			}
		...
		void hideResults()
		{
			...
			lbCategoryAutoComplete.Visible = false;
			lbBrandAutoComplete.Visible = false;
		}
		...
		void addWallet(string walletName, int i)
		{
			...
			//System.Windows.Forms.TableLayoutPanel tableLayoutPanelPaymentMethods = new TableLayoutPanel();
			//this.panelPaymentWays.Controls.Add(tableLayoutPanelPaymentMethods);
			//tableLayoutPanelPaymentMethods.Dock = System.Windows.Forms.DockStyle.Top;
			//tableLayoutPanelPaymentMethods.ColumnCount = 3;
			//tableLayoutPanelPaymentMethods.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle(System.Windows.Forms.SizeType.Percent, 60F));
			//tableLayoutPanelPaymentMethods.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle(System.Windows.Forms.SizeType.Percent, 30F));
			//tableLayoutPanelPaymentMethods.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle(System.Windows.Forms.SizeType.Percent, 10F));
			//tableLayoutPanelPaymentMethods.Controls.Add(label, 0, 0);
			//tableLayoutPanelPaymentMethods.Controls.Add(textBox, 1, 0);
			//tableLayoutPanelPaymentMethods.Controls.Add(delete, 2, 0);
			//tableLayoutPanelPaymentMethods.Location = new System.Drawing.Point(3, 3);
			//tableLayoutPanelPaymentMethods.Name = "tableLayoutPanelPaymentMethod";
			//tableLayoutPanelPaymentMethods.RowCount = 1;
			//tableLayoutPanelPaymentMethods.RowStyles.Add(new System.Windows.Forms.RowStyle(System.Windows.Forms.SizeType.Percent, 100F));
			//tableLayoutPanelPaymentMethods.Size = new System.Drawing.Size(331, 40);
			//tableLayoutPanelPaymentMethods.TabIndex = 5;
			...
			//delete.Anchor = System.Windows.Forms.AnchorStyles.Left;
		...
		void setLocation(List<(int, Label)> labels, List<(int, NumericUpDown)> textBoxes, List<(int, Button)> buttons)
		{
			for (int i = 0; i < labels.Count; i++)
			{
				...
				//labels[i].Item2.Anchor = ((System.Windows.Forms.AnchorStyles)((System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Left)));
				textBoxes[i].Item2.Location = new System.Drawing.Point(120, 10 + i * 22);
				//textBoxes[i].Item2.Anchor = ((System.Windows.Forms.AnchorStyles)((System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Right)));
				buttons[i].Item2.Location = new System.Drawing.Point(201, 10 + i * 22);
				//buttons[i].Item2.Anchor = ((System.Windows.Forms.AnchorStyles)((System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Right)));
			}
		}
		...
		private void btnSave_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				for (int i = 0; i < isAddedTextBox.Count; i++)
				{
					...
					CUD(sqlUpdateAmount);
					string sqldgHistory = @"INSERT INTO DigitalWallet_History(DigitalWalletId, TransactionType, DetailPurpose,
					TransactedAmount, UpdatedDate, BalanceSheetId) VALUES ("+digitalWalletId+", 'WITHDRAWN', 'INVENTORY PURCHASE PAYMENT'," +
					"" + transactionAmount + ", '" + dT + "', " + balanceSheetId + ")";
					CUD(sqldgHistory);
					...
					string sql = @"INSERT INTO Inventory_Payment_Methods(InventoryId, DigitalWalletId, TransactionAmount, BalanceSheetId, UpdatedDate, DG_HistoryId)
									VALUES(" + inventoryId + ", " + digitalWalletId + ", " + transactionAmount + ", " + balanceSheetId + ", '"+ dT +"', "+ dgHistoryId +")";
					CUD(sql);
				...
				foreach (DataGridViewRow d in dgProductItems.Rows)
				{
					...
					string brand = d.Cells[2].Value.ToString();
					int brandId = 0;
					
					if(allBrand.Values.Contains(brand) == true)
                    {
						brandId = allBrand.FirstOrDefault(x => x.Value == brand).Key;
                    }

					if (allBrand.Values.Contains(brand) == false)
					{
						string sqlInsertBrand = "INSERT INTO Brand(Title) VALUES('" + brand + "')";
						CUD(sqlInsertBrand);
						brandId = Convert.ToInt32(getMaxId("SELECT MAX(ID) FROM Brand"));
					}

					string category = d.Cells[3].Value.ToString();
					int categoryId = 0;

					if (allCategory.Values.Contains(category) == true)
					{
						categoryId = allCategory.FirstOrDefault(x => x.Value == category).Key;
					}

					if (allCategory.Values.Contains(category) == false)
					{
						string sqlInsertCategory = "INSERT INTO Category(Title) VALUES('" + category + "')";
						CUD(sqlInsertCategory);
						categoryId = Convert.ToInt32(getMaxId("SELECT MAX(ID) FROM Category"));
					}
					double buyingPrice = Convert.ToDouble(d.Cells[4].Value.ToString());
					double discountIndividual = Convert.ToDouble(d.Cells[5].Value.ToString());
					double quantity = Convert.ToDouble(d.Cells[6].Value.ToString());
					long unitId = Convert.ToInt64(d.Cells[7].Value.ToString());
					double salePrice = Convert.ToDouble(d.Cells[8].Value.ToString());
					double orgPrice = (buyingPrice * quantity) - discountIndividual;

					double costPerUnit = Convert.ToDouble(d.Cells[9].Value.ToString());
					...
					double priceAfterDiscount = Convert.ToDouble(d.Cells[10].Value.ToString());
					double GivenVat = 0;
					...
					if (true)
					{
						...
						string sql = @"INSERT INTO Products(Name, CategoryId, BrandId, ItemCode, Price, Vat, VatType, TotalQuantity, Discount, PriceAfterDiscount, CostPerUnit, UnitID, CreateDate, UpdateDate, CreatedBy) 
					    VALUES('" + name + "', " + categoryId + ", " + brandId + ", '" + itemCode + "', " + buyingPrice + ", " + vatAll +
							", '" + vatType + "', " + quantity + ", " + discountIndividual + "," + priceAfterDiscount + ", " + costPerUnit + ", " + unitId + ", '" + dT + "', '" + DateTime.Now + "', 1)";

						CUD(sql);
						long productId = getMaxId("SELECT MAX(ID) FROM Products");
						string sqlDuplicate = @"INSERT INTO Products_1(ID, Name, CategoryId, BrandId, ItemCode, Price, Vat, VatType, TotalQuantity, Discount, PriceAfterDiscount, CostPerUnit, UnitID, CreateDate, UpdateDate, CreatedBy) 
					    VALUES(" + productId + ", '" + name + "', " + categoryId + ", " + brandId + ", '" + itemCode + "', " + buyingPrice + ", " + vatAll +
							", '" + vatType + "', " + quantity + ", " + discountIndividual + "," + priceAfterDiscount + ", " + costPerUnit + ", " + unitId + ", '" + dT + "', '" + DateTime.Now + "', 1)";
						...
						CUD(sql1);
						long productDetailsId = getMaxId("SELECT MAX(ID) FROM ProductDetails");
						string sql1Duplicate = @"INSERT INTO ProductDetails_1(ID, ProductID, UnitID, SellingPrice, GivenVat, RemainQuantity, SupplierID, InventoryId) VALUES(" + productDetailsId + ", " + Convert.ToInt64(id1) + ", " + unitId + ", " + salePrice + "," + GivenVat + ", " + quantity + ", " + supplierId + ", " + inventoryId + " )";
						CUD(sql1Duplicate);
						Clear();
		...
		private void EntryItems_Load(object sender, EventArgs e)
		{
			...
			cmbUnitForBuy.SelectedIndex = cmbUnitForBuy.Items.Count > 0 ? 1 : -1;
  ...
  //          FROM (((Products AS p LEFT JOIN ProductDetails AS d ON p.ID = d.ProductId) LEFT JOIN Units AS u ON d.UnitId = u.ID)  LEFT JOIN Units AS n ON p.UnitId = n.ID)
		...
		private void groupBox1_Enter(object sender, EventArgs e)
		{
			...
			if(txtName.Text != "") lblForNameFieldValidation.Visible = false;
  ...
  //          FROM (((Products AS p LEFT JOIN ProductDetails AS d ON p.ID = d.ProductId) LEFT JOIN Units AS u ON d.UnitId = u.ID)  LEFT JOIN Units AS n ON p.UnitId = n.ID)
		...
		{
			//object listBox = sender.GetType();
			//bool i = lbAutoCompleteSupplier.MouseCaptureChanged;
			//object f = e.GetType();
			//if (listBox != lbAutoCompleteSupplier)
			...
			if(!lbAutoCompleteSupplier.Focused)
			{
				/*if(lbAutoCompleteSupplier.Click == true)*/
				if (lbAutoComplete.Visible) lbAutoComplete.Visible = false;
        ...
        void calculation()
		{
			...
			if (dgProductItems.Rows.Count > 0)
			{
				...
				foreach (DataGridViewRow d in dgProductItems.Rows)
				{
					buyingPrice = Convert.ToDouble(d.Cells[4].Value.ToString());
					discountIndividual = Convert.ToDouble(d.Cells[5].Value.ToString());
					buyingQuantity = Convert.ToDouble(d.Cells[6].Value.ToString());
					vat = Convert.ToDouble(d.Cells[11].Value.ToString());
					//totalVat = totalVat + (Convert.ToDouble(d.Cells[8].Value.ToString())* buyingQuantity * vat / 100);
					totalVat = totalVat + Convert.ToDouble(d.Cells[13].Value.ToString());
				...
				foreach (DataGridViewRow d in dgProductItems.Rows)
				{
					...
					d.Cells[9].Value = costPerUnit;
					d.Cells[10].Value = costPerUnitAfterDiscount;
			...
			if(due >= 0) txtDue.Value = Convert.ToDecimal(due);
		...
		{
			if (txtName.Text != "" && txtBuyingTotalQuantity.Value > 0)
			{
				ItemGrid item = new ItemGrid();
				item.ItemCode = txtCode.Text;
				item.Name = txtName.Text;
				...
				cmbUnitForBuy.SelectedIndex = 1;
				item.UnitId = Convert.ToInt16(units.ElementAt(cmbUnitForBuy.SelectedIndex - 1).Key);
            ...
            {
				MessageBox.Show("Please, Insert Product Name & Quantity!");
            }
		...
		void dgProductItemLoad()
		{
			...
			dgProductItems.Columns.Add("cName", "Model");
			dgProductItems.Columns.Add("cBrand", "Brand");
			dgProductItems.Columns.Add("cCategory", "Category");
		...
		void LoadGrid(ItemGrid item)
		{
			...
			dgProductItems.Rows.Add(item.ItemCode, item.Name, item.Brand, item.Category, item.Price, item.Discount, item.TotalQuantity, item.UnitId, item.SalePrice, item.CostPerUnit, item.CostPerUnitAfterDiscount, item.Vat, item.VatType, item.GivenVat * item.TotalQuantity);
			dgProductItems.AllowUserToAddRows = false;
		...
		private void dgProductItems_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.ColumnIndex == 12 && e.RowIndex > -1)
			{
				...
				//double vat = Convert.ToDouble(dgProductItems.Rows[e.RowIndex].Cells[7].Value);r
		...
		private void itemCodeSorting()
        {
			long productId = getProductId() + 1;
			for(int i = 0; i < dgProductItems.Rows.Count; i++)
            {
				dgProductItems.Rows[i].Cells[0].Value = "item-" + productId;
				productId++;
        ...
        private void radioBtnInclusive_CheckedChanged(object sender, EventArgs e)
        {
            if (radioBtnInclusive.Checked)
            {
				radioBtnExclusive.Checked = false;
        ...
        private void radioBtnExclusive_CheckedChanged(object sender, EventArgs e)
        {
			if (radioBtnExclusive.Checked)
			{
				radioBtnInclusive.Checked = false;
				radioBtnDutyFree.Checked = false;
			}
        ...
        private void Products_Resize(object sender, EventArgs e)
        {
            ...
            {
				panel1.Size = new Size(699, this.Size.Height);
        ...
        {
			if (panel1.Size.Width < 700)
			{
				panel1.Size = new System.Drawing.Size(700, 500);
        ...
        private void groupBox1_MouseCaptureChanged(object sender, ControlEventArgs e)
        {
			object listBox = sender.GetType();
			bool i = lbAutoCompleteSupplier.IsHandleCreated;
			object f = e.GetType();
			int a = 0;
		}
        ...
        {
			if (txtCategory.Focused)
			{
				lbCategoryAutoComplete.Items.Clear();
				lbCategoryAutoComplete.Visible = true;
				...
				{
					string pattern = @"\b\w*" + txtCategory.Text.Trim() + @"+\w*\b";
					lbCategoryAutoComplete.Items.Clear();

					foreach (KeyValuePair<int, string> s in allCategory.Where(p => Regex.IsMatch(p.Value, pattern, RegexOptions.IgnoreCase)))
					{
						lbCategoryAutoComplete.Items.Add(s.Value);
						categoryIds.Add(s.Key);
					}
					if (lbCategoryAutoComplete.Items.Count > 0)
					{
						lbCategoryAutoComplete.Visible = true;
					}
					else
        ...
        private void lbCategoryAutoComplete_SelectedIndexChanged(object sender, EventArgs e)
        {
			if (lbCategoryAutoComplete.SelectedIndex > -1)
			{
				int selectedIndex = lbCategoryAutoComplete.SelectedIndex;
				categoryId = categoryIds[selectedIndex];
				txtCategory.Text = lbCategoryAutoComplete.Items[selectedIndex].ToString();
			}
        ...
        private void txtBrand_TextChanged(object sender, EventArgs e)
        {
			if (txtBrand.Focused)
			{
				lbBrandAutoComplete.Items.Clear();
				lbBrandAutoComplete.Visible = true;
				brandIds = new List<long>();
				//productId = 0;
				...
				{
					string pattern = @"\b\w*" + txtBrand.Text.Trim() + @"+\w*\b";
					lbBrandAutoComplete.Items.Clear();

					foreach (KeyValuePair<int, string> s in allBrand.Where(p => Regex.IsMatch(p.Value, pattern, RegexOptions.IgnoreCase)))
					{
						lbBrandAutoComplete.Items.Add(s.Value);
						brandIds.Add(s.Key);
					}
					if (lbBrandAutoComplete.Items.Count > 0)
					{
						lbBrandAutoComplete.Visible = true;
					}
					else
        ...
        private void lbBrandAutoComplete_SelectedIndexChanged(object sender, EventArgs e)
        {
			if (lbBrandAutoComplete.SelectedIndex > -1)
			{
				int selectedIndex = lbBrandAutoComplete.SelectedIndex;
				brandId = brandIds[selectedIndex];
				txtBrand.Text = lbBrandAutoComplete.Items[selectedIndex].ToString();
			}
